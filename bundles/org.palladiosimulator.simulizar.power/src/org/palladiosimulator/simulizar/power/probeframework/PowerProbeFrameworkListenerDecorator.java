 ...
 * initialize sliding-window based (i.e., {@link TimeDriven}) computations of
 * power and energy consumption (based on utilization measurements).
 * 
...
public class PowerProbeFrameworkListenerDecorator extends AbstractRecordingProbeFrameworkListenerDecorator {
	 ...
	 * measurements. First gets the monitored elements from the monitor repository,
	...
	private void initPowerMeasurements(final Collection<MeasurementSpecification> powerMeasurementSpecs) {
		...
		if (!powerMeasurementSpecs.isEmpty()) {
			...
			// Register simulation time provider. This is used to calculate transitions
	...
	/**
	 * Checks whether the given measurement specification is valid. If this methods
	 * returns, then it is valid:<br>
	 * For validity, it is required that
	 * <ul>
	 * <li>the associated power providing entity exist (i.e., non-null),</li>
	 * <li>the passed optional be not empty, i.e., the ProcessingType specified in
	 * the measurement specification must be a {@link TimeDrivenAggregation}.</li>
	 * </ul>
	 * 
	 * @param powerMeasurementSpec A {@link MeasurementSpecification} for power
	 *                             measurements.
	 * @param powerProvidingEntity The {@link PowerProvidingEntity} associated with
	 *                             the given measurement specification.
	 * @param aggregation          An {@link Optional} that contains the
	 *                             {@link TimeDrivenAggregation}
	 *                             {@link ProcessingType} that has been extracted
	 *                             from the given measurement specification.
	 * 
	 * @throws IllegalStateException Iff
	 *                               {@code powerProvidingEntity == null || !aggregation.isPresent()}.
	 */
